...
```mermaid
graph LR
  TODO["todo"] -.-> IMP["implementing"] -.-> REV["in-review"] -.-> APP["approved"] -.-> DEP["deploying"] -.-> DONE["done"]

  style TODO fill:#ebe5d9,stroke:#6b5b3e,stroke-width:2px,color:#2c2416
  style IMP fill:#fef3e2,stroke:#b45309,stroke-width:2.5px,color:#b45309
  style REV fill:#fef3e2,stroke:#b45309,stroke-width:2.5px,color:#b45309
...
| Status | What's happening | Who's responsible |
|---|---|---|
| **todo** | Synced from tracker, waiting for user to trigger | User |
| **implementing** | Agent writing code, running tests, opening PR | Implementation Agent |
| **in-review** | AI review, human review, risk assessment | Review + Risk Agents |
| **approved** | All gates passed, ready to merge and deploy | System |
| **deploying** | Merging, deploying to envs, monitoring | Deploy + Monitor Agents |
| **done** | In prod, monitoring passed | - |
| **failed** | Error at any stage | Needs attention |
| **blocked** | Needs human intervention | User |

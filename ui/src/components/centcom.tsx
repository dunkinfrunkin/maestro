      ...
      <div className="border border-border rounded-md overflow-hidden">
        <div className="px-3 py-2 border-b border-border bg-surface flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] uppercase tracking-widest font-semibold">Polled MRs</span>
            <span className="text-[10px] text-muted">({metrics.polled_mrs.length})</span>
          </div>
        </div>
        {metrics.polled_mrs.length === 0 ? (
          <div className="px-3 py-4 text-center">
            <div className="text-[10px] uppercase tracking-widest text-muted">No MRs being polled</div>
          </div>
        ) : (
          <table className="w-full text-[11px]">
            <thead>
              <tr className="border-b border-border text-[9px] uppercase tracking-widest text-muted">
                ...
                <th className="text-left px-3 py-1.5 font-medium">PR</th>
                <th className="text-left px-3 py-1.5 font-medium">Status</th>
                <th className="text-right px-3 py-1.5 font-medium">Last Checked</th>
              </tr>
            </thead>
            <tbody>
              {metrics.polled_mrs.map((mr) => (
                <tr
                  key={mr.external_ref}
                  className="border-b border-border last:border-0 hover:bg-surface-hover transition-colors cursor-pointer"
                  onClick={() => router.push(`/tasks/${encodeURIComponent(mr.external_ref)}`)}
                >
                  <td className="px-3 py-2 font-semibold">{mr.repo ? mr.repo.split("/").pop() : "—"}</td>
                  ...
                  <td className="px-3 py-2 text-muted uppercase text-[10px]">{mr.status.replace("_", " ")}</td>
                  <td className="px-3 py-2 text-right text-muted">
                    {mr.last_checked_at
                      ? formatDuration((Date.now() - new Date(mr.last_checked_at).getTime()) / 1000) + " ago"
                      : "never"}
                  </td>
                </tr>
              ))}
